...
import org.src.pieces.Colour;
import org.src.pieces.Piece;
...
public ArrayList<String> movePawn(Piece[][] board,Piece tpiece) {
	...
	// Verific toate miscarile posibile care le poat face cu org.src.pieces.PAWN Back
	// Enpassant
	if(board[i][j].colour.equals(Colour.Black) && (i - 1) != 0) {
		if((j+1) <= 8 && board[i][j+1] != null)
			if (i == 4 && board[i][j+1].getType().equals("org.src.pieces.PAWN") && (board[i][j+1].nrMoves == 1)) {
	...
	// Verific toate miscarile posibile care le poat face cu org.src.pieces.PAWN White
	// Enpassant
	if(board[i][j].colour.equals(Colour.White) && (i+1) != 9) {
		if((j+1) <= 8 && board[i][j+1] != null)
			if (i == 5 && board[i][j+1].getType().equals("org.src.pieces.PAWN") && (board[i][j+1].nrMoves == 1)) {
				if (board[i+1][j+1] == null && board[i+2][j+1] == null)
					s.add(moveToString(j, i, j+1, i+1, "e"));
